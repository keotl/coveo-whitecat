...
public class Networking {
    ...
    static GameMap deserializeGameMap(String inputString, int[][] productions, GameMap map) {
        String[] inputStringComponents = inputString.split(" ");
		...
		map.getLocation(a,b).getSite().production = productions[a][b];
            }
        }
    ...
    static InitPackage getInit() {
        ...
        deserializeGameMap(getString(),productions, map);
    ...
    static void updateFrame(GameMap map) {
        map.reset();
	int[][] productions = deserializeProductions(getString(), map.width, map.height);
        deserializeGameMap(getString(), productions, map);
    }
